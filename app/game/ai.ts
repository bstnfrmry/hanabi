 ...
 */
function isCardPossible(card: Card, possibleCards: Card[]): boolean {
  return (
...
function isCardDangerous(card: Card, state: GameState): boolean {
...
function isCardEverPlayable(card: Card, state: GameState): boolean {
 ...
 */
function isCardDiscardable(card: IHiddenCard, state: GameState): boolean {
  ...
  colors.forEach(color => {
    numbers.forEach(number => {
      if (
        ...
        isCardPossible({ color, number } as Card, possibleCards)
      ) {
        deductions.push({
...
function getPossibleCards(state: GameState, player: number): Card[] {
 ...
 *
 */
export interface IDeduction extends Card {
  deductionLevel: number;
}

export type IGameView = GameState & { gameViews: PlayerView[] };
...
export function gameStateToGameView(gameState: GameState): IGameView {
  // copy the state
  ...
  Object.values(state.players).forEach((player: Player, i) => {
    const gameView = { hand: [] };
...
export function commitViewAction(state: IGameView, action: Action): IGameView {
...
export function chooseAction(state: IGameView): Action {
...
export default function play(state: GameState): GameState {
