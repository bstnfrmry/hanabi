...
const startingHandSizeMulticolor6 = { 2: 6, 3: 6, 4: 6, 5: 6 };
...
export function emptyHint(options: IGameOptions): ICardHint {
  return {
    color: {
      ...
      [IColor.WHITE]: 1,
      [IColor.MULTICOLOR]:
        options.variant === GameVariant.MULTICOLOR || options.variant === GameVariant.MULTICOLOR6 ? 1 : 0,
...
export function joinGame(state: IGameState, player: IPlayer): IGameState {
  ...
  const handSize =
    game.options.variant === GameVariant.MULTICOLOR6
      ? startingHandSizeMulticolor6[game.options.playersCount]
      : startingHandSize[game.options.playersCount];
  const hand = game.drawPile.splice(0, handSize);
...
export function newGame(options: IGameOptions): IGameState {
  ...
  if (options.variant === GameVariant.MULTICOLOR || options.variant === GameVariant.MULTICOLOR6) {
    cards.push(
