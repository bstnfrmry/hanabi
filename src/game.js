...
const { flatten, flatMap, groupBy, last, padEnd, sumBy, without } = require('lodash')
...
function colorValue(color, value, knowledge = {}) {
  ...
  if (knowledge.full) { string = string.inverse }
  else if (knowledge.value) { string = string.underline }
  else if (knowledge.color) { string = string.italic }
...
class Player {
  ...
  hintColor(player, color) {
    ...
    this.game.log(`# Player ${this.name} gave ${color} color hint to ${player.name}`)
  ...
  hintValue(player, value) {
    ...
    this.game.log(`# Player ${this.name} gave ${value} value hint to ${player.name}`)
  ...
  guess(card) {
    let remainingCards = without(this.game.cards,
...
class Game {
  constructor(options) {
    ...
    // Initialize cards
    this.cards = flatMap([Color.BLUE, Color.WHITE, Color.RED, Color.YELLOW, Color.GREEN], color => ([
  ...
  async play(onTick) {
    ...
    switch (action.type) {
      ...
      case 'hint-value':
        ...
        break
      case 'play': this.currentPlayer.play(action.card)
        break
      case 'discard': this.currentPlayer.discard(action.card)
        break
    }

    if (this.state.over) {
      this.log(`# Game over - ${this.state.reason} - Score: ${this.score} / ${this.maxScore}`)
      this.print()
      return
    }
    ...
    this.players.forEach(player => {
      player.hand.forEach((card, position) => {
        card.age += 1
        if (position === 0) {
          card.ageInDiscardPile = card.ageInDiscardPile ? card.ageInDiscardPile + 1 : 1
        } else {
  ...
  print() {
    ...
    this.log([
      ...
      `Board   ${boardPiles.map(color => {
        const topValue = this.board[color].length ? last(this.board[color]).value : 0
        return colorValue(color, topValue)
      }).join(' ')}`,
      `Discard ${flatten(Object.values(this.discardPile)).map(card => card.text).join(' ')}`,
      ...this.players.map(player => {
        return `${padEnd(player.name, 7)} ${player.hand.map(card => card.text).join(' ')} ${player === this.currentPlayer ? '*' : ''}`
      }),
      '---'
    ].join('\n'))
  }
