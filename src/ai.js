...
function lastActableHints(player) {
  return player.receivedHints.filter(hint => {
    ...
    if (hint.cards.length === 1 && positionDuringHint === 0) {
      return false;
    }

    // Exclude if I know fully know the card and can't play it
    if (card.knowledge.full && !player.game.canPlay(card)) {
      return false;
    }
...
async function ai({ game, player }) {
  ...
  const hints = flatMap(["color", "value"], hintType =>
    flatMap(player.teammates, teammate => {
      return teammate.hand
        .filter((card, position) => {
          ...
          const affectedCards = teammate.hand.filter(
            item => item[hintType] === card[hintType]
          );
          const willReveal = affectedCards.filter(
            card => card.knowledge.partial && !card[hintType]
          );
          ...
          if (
            player.teammates.find(teammate =>
              teammate.hand.find(item => {
                return (
        ...
        .map(card => {
          const actableCards = teammate.hand.filter(card => card.knowledge.full)
            .length;
          const sideEffects = without(teammate.hand, card).filter(
            sideCard =>
              !sideCard.knowledge[hintType] &&
              sideCard[hintType] === card[hintType]
          );
          const playableSideEffects = uniqBy(
            sideEffects,
            card => `${card.color}.${card.value}`
          ).filter(sideCard => game.canPlay(sideCard));
          const actableHints = lastActableHints(teammate);
          const nextHintedCard =
            actableHints.length &&
            actableHints[0].cardsPlayed === 0 &&
            last(actableHints.cards);
          const inNextHintedCards = actableHints.find(item => item === card);
          const cardPosition = teammate.hand.indexOf(card);

          const score =
            (cardPosition === 0 &&
            !game.canDiscard(card) &&
            card.ageInDiscardPile >= 8 &&
            ...
            (card.knowledge.partial && !card[hintType] // Buff is card would then be fully reveals
              ? game.canDiscard(card)
                ? 0.8
                : 0.3 // Buff furthermore if card can be discarded
              : 0) +
  ...
  if (game.remainingNoteTokens && topHint && topHint.score >= 1.7) {
    return {
      ...
      ...(topHint.hintType === "color" && {
        type: "hint-color",
        color: topHint.card.color
      }),
      ...(topHint.hintType === "value" && {
        type: "hint-value",
        value: topHint.card.value
      })
    };
  }
  ...
  const deducedCards = player.hand.filter(
    card =>
      uniqBy(
        card.guesses,
        item => item.color === card.color && item.value === card.value
      ).length === 1
  );
  ...
  if (deducedCard) {
    return {
      type: "play",
      card: deducedCard,
      reason: "knowledge by deduction"
    };
  }

  // 4 - I have a hint I can act upon
  const actableHints = lastActableHints(player)
    .reverse()
    .map(hint => {
      const [card] = hint.cards.slice(-hint.cardsPlayed - 1);
  ...
  const [{ hint, card, score: hintScore } = {}] = sortBy(
    actableHints,
    hint => -hint.score
  );
  ...
  const dicardableCard = [...hintedCards, ...deducedCards].find(card =>
    game.canDiscard(card)
  );
  if (dicardableCard) {
    return {
